...
public final class QuoridorGame extends Game {
    ...
    public void start() {
        println("\n=== Quoridor ===");
        println("Goal: be the first to reach the opposite edge.");
        println("Commands: type 'move' to relocate your pawn, or 'place' to position a wall.");
        println("Moves require selecting a destination (row col) followed by 'enter'. For walls, use WASD to shift, 'turn' to rotate, 'enter' to confirm, 'change' to re-choose.\n");
        ...
        while (true) {
            ...
            if (board.hasPlayerWon(opponent)) { announceWinner(board, opponent, pawnColors); return; }
        ...
        while (true) {
            ...
            if (mode == ActionMode.NONE) {
                if (cmd.equals("move")) {
                    ...
                    println(WARN + "No walls remaining; cannot place." + RESET);
                        continue;
                    }
            ...
            if (cmd.equals("turn") && mode == ActionMode.PLACE) {
                if (!pendingWall.ready()) {
                    println(WARN + "No wall preview to rotate." + RESET);
                    continue;
                }
            ...
            if (mode == ActionMode.PLACE && (cmd.equals("w") || cmd.equals("a") || cmd.equals("s") || cmd.equals("d"))) {
                if (!pendingWall.ready()) {
                    println(WARN + "No wall preview to move." + RESET);
                    continue;
                }
                int dr = 0, dc = 0;
                switch (cmd) {
